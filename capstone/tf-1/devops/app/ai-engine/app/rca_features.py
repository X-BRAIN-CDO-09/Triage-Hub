...
def extract_rca_features(request: Any, rca: dict[str, Any], include_metadata: bool = False) -> dict[str, float]:
    ...
    features["runbook_count"] = float(
        len(request.ownership.runbooks) if request.ownership and request.ownership.runbooks else 0
    )
...
def series_features_for_service(metrics: list[Any], service: str, prefix: str) -> dict[str, float]:
    ...
    for series in metrics:
        ...
        if is_latency_metric(name):
            ...
            features[f"{prefix}_latency_window_ratio_max"] = max(
                features[f"{prefix}_latency_window_ratio_max"], window_ratio
            )
            ...
            if baseline_mean > 0:
                ...
                features[f"{prefix}_workload_baseline_max"] = max(
                    features[f"{prefix}_workload_baseline_max"], baseline_mean
                )
...
def alert_metadata_features(request: Any) -> dict[str, float]:
    ...
    return {
        ...
        "alert_latency_token": 1.0
        if any(token in text for token in ("latency", "p95", "p90", "duration", "timeout", "delay", "slow"))
        else 0.0,
        "alert_down_token": 1.0 if any(token in text for token in ("down", "unavailable", "loss", "critical")) else 0.0,
        "alert_noisy_token": 1.0
        if any(token in text for token in ("noisy", "flapping", "false", "ambiguous"))
        else 0.0,
    }
...
def metric_families_from_text(text: str) -> list[str]:
    ...
    if any(
        token in text for token in ("traffic loss", "traffic_loss", "workload", "throughput", "request", "qps", "rps")
    ):
