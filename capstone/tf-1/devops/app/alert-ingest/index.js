...
// SQS batch: process each record independently and report per-record
// failures via batchItemFailures so a single bad record doesn't force
// the whole batch (including already-successful records) to retry.
async function handleSqsBatch(event) {
  ...
  for (const record of event.Records) {
    try {
      const result = await handleApiGatewayLikeEvent(
        buildEventFromSqsRecord(record),
      );
      results.push(result);
    } catch (err) {
...
async function handleApiGatewayLikeEvent(event) {
  ...
  try {
    ...
    return jsonResponse(400, { error: `Invalid JSON body: ${err.message}` });
  }
  ...
  for (const alert of alerts) {
    ...
    // All per-alert validation failures are soft-fails: record the
    // reason in `dropped` and move on to the next alert, rather than
    // throwing and aborting the whole batch. Only truly fatal,
    // request-level problems (missing body, bad JSON, missing env
    // vars) should abort the whole invocation — see checks above.
    ...
    let tenant;
    try {
      tenant = await getTenantConfig(tenantId, tenantCache);
    } catch (err) {
      // Infra-level failure (e.g. DynamoDB unavailable) is not the
      // alert's fault — let it propagate so the caller/queue retries.
    ...
    try {
      ...
      // Failing to enqueue is also infra-level — surface it rather
      // than silently dropping a validated alert.
      console.error("Failed to send incident seed to SQS", {
        tenant_id: tenantId,
...
async function sendSeedToSqs(seed) {
  ...
  const command = new SendMessageCommand({
    ...
    // Dedup on the deterministic correlation_id (derived from
    // tenant + alertname + fingerprint) so Lambda/SQS retries of the
    // same alert don't create duplicate downstream incidents.
    MessageDeduplicationId: seed.correlation_id,
    MessageAttributes: {
